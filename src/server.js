...
import {
  MODES, UNIVERSES, getUniverse, sanitizeSettings, isEndless,
...
/** Um dataset por universo: data/pokemon.json, data/bleach.json, ... */
const DATASETS = new Map();
for (const universe of Object.values(UNIVERSES)) {
  const file = path.join(ROOT, 'data', universe.dataFile);
  const list = JSON.parse(fs.readFileSync(file, 'utf8'));
  DATASETS.set(universe.id, { list, byId: new Map(list.map(item => [item.id, item])) });
  console.log(`${universe.label}: ${list.length} itens (${list.filter(i => i.eligible).length} sorteáveis)`);
}

const app = express();
const isProd = process.env.NODE_ENV === 'production';
app.use(express.static(path.join(ROOT, 'public'), { maxAge: isProd ? '1h' : 0 }));
app.get('/api/dataset/:universe', (req, res) => {
  const dataset = DATASETS.get(req.params.universe);
  if (!dataset) return res.status(404).json({ error: 'Universo desconhecido.' });
...
const datasetOf = (room) => DATASETS.get(room.settings.universe) ?? DATASETS.get(getUniverse().id);
